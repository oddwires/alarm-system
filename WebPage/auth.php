...
//error_log('Remember me: '.$_POST['rememberme']);
    ...
    if($loginisvalid){
        ...
        if ($_SESSION['badcount'] == 1) { $data = array("status" => "Failed #1"); }
        if ($_SESSION['badcount'] == 2) { $data = array("status" => "Failed #2"); }
        if ($_SESSION['badcount'] >= 3) { $data = array("status" => "Failed #3"); }     // thank you and goodnight.
...
//error_log('badcount: '.$_SESSION['badcount']);            //DEBUG
